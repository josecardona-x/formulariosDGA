...
public class FlowService {
    ...
    public void createFlow(String requestId) throws Exception {
        ...
        eventLogFacade.insertInAudit(form.getId(), "", "El proceso que se seguirá es: " + processCode, "WORKFLOW", userCretedBy);
    ...
    public void sendStep(String requestId) {
        ...
        for (sv.gob.aduana.entity.Request r : form.getRequestCollection()) {
            ...
            if (RequestTypeService.isExtern(form.getFormType())) {
                if (r.getPerson().getCodedeclarant() != null) {
                    ...
                    strBuilder.append("<br/>Resolución: ");
            ...
            if (!RequestTypeService.isRequestWithPermissionsFilled(r.getTyperequest().getId())) {
                ...
                if (r.getProfileCollection().size() > 0) {
                    for (sv.gob.aduana.entity.Profile p : r.getProfileCollection()) {
                        ...
                        strBuilder.append("<br/><br/>Acción: ");
                ...
                if (r.getOtherCollection().size() > 0) {
                    for (sv.gob.aduana.entity.Other o : r.getOtherCollection()) {
                        ...
                        strBuilder.append("<br/><br/>Información: ");
            ...
            if (System.getProperty("custom.enviroment.oim").equals("TEST")) {
                ...
                title = "[TEST] - Solicitud de servicios informáticos para accesos - " + form.getId() + " - " + form.getStep();
            } else {
                strBuilder.append("http://identidad.mh.gob.sv/custom-app/dga/form/ApproveByMail?hashcode=");
                title = "Solicitud de servicios informáticos para accesos - " + form.getId() + " - " + form.getStep();
            }
        ...
        if (existRequestAvailable) {
            ...
            email.setMessage("Todos las solicitudes de esta petición fueron rechazados, favor verifique en el portal para verificar el detalle.");
    ...
    public void rejectFlow(String requestId) {
        try {
            ...
            eventLogFacade.insertInAudit(request.getForm().getId(), request.getId(), "El formulario será rechazado, ya que no existen otras solicitudes",
                    "WORKFLOW", "");
            ...
            for (Request r : form.getRequestCollection()) {
                for (Requestflow rf : requestflowFacade.getByRequestId(r.getId())) {
                    ...
                    rf.setStatus("FINISH");
                    rf.setUser("-");
                    rf.setComment("Rechazado el paso automáticamente debido a que un paso anterior fue rechazado.");
        ...
        try {
            ...
            for (sv.gob.aduana.entity.Request r : form.getRequestCollection()) {
                ...
                if (RequestTypeService.isNew(r.getTyperequest().getId()) || RequestTypeService.isEnabled(r.getTyperequest().getId())) {
                    strBuilder.append("</td></tr><tr><td><strong>Contraseña: </strong></td><td>");
                    strBuilder.append(r.getPassword());
                } else {
                    strBuilder.append("</td></tr><tr><td><strong>Contraseña (Si aplica cambio): </strong></td><td>");
                    strBuilder.append(r.getPassword());
                }
                ...
                strBuilder.append("<br/><h5>Flujo de aprobación</h5><br/><table width='100%'>");
                strBuilder.append("<tr>");
            ...
            if (System.getProperty("custom.enviroment.oim").equals("TEST")) {
                title = "[TEST] - Solicitud de servicios informáticos para accesos - " + form.getId() + " - Finalización de formulario ";
            } else {
                title = "Solicitud de servicios informáticos para accesos - " + form.getId() + " - Finalización de formulario ";
            }
